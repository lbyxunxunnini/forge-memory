...
version: 0.4.8
summary: 让 AI 编程助手记住你的项目。三条命令初始化，之后 AI 自动理解项目结构、找到相关文件、知道最近改了什么。支持 chunk 级元数据、review 状态治理、质量门禁、多分支隔离、会话记忆恢复和一键诊断。
tags: [memory, project, context, workflow, index]
license: MIT
name: forge-memory
description: 项目记忆层，负责项目认知和上下文供给。三级触发：完整触发（项目全貌需求时执行扫描/初始化/上下文生成）、被动读取（局部任务时读取 project-summary.md 加速代码定位）、不触发。支持分支隔离存储、Git 历史、SQLite 索引。初始化、扫描、状态查询、上下文生成、影响分析、会话记忆均由此 skill 管理。
---
...
flowchart TD
    A[收到请求] --> B{触发层级判断}
    B -->|完整触发| D{四状态选择}
    B -->|被动读取| P[读取 project-summary.md]
    B -->|不触发| C[按常规任务处理]
    D -->|优先级1| E[总结会话摘要]
    D -->|优先级2| F[读取会话记忆]
    D -->|优先级3| G[读取项目结构化文件]
...
当 `.project-context/project-summary.md` 存在时，处理以下任务前**先读取该文件**作为项目结构参考，加速代码定位，但**不触发完整工作流**（不扫描、不初始化、不写会话摘要）：

- 局部 bug 排查（如"试听功能无效""登录按钮没反应"）。
- 特定模块/页面的代码修改（如"修改音色列表的排序逻辑"）。
- 涉及项目中某个功能的代码解释或分析。
- 需要在项目中定位某类文件或功能的实现位置。

读取后，智能体应从 project-summary.md 中提取相关模块路径，再定向读取源码，而非盲扫整个项目。

### 不触发

- 只是遇到大型项目但不需要上下文。
- 用户只是说"总结一下今天的会话"等与项目代码无关的任务。
- `.project-context/project-summary.md` 不存在且用户未要求初始化。
...
**被动读取示例**：
1. 用户说"帮我排查试听功能的 bug" → 被动读取 project-summary.md 定位"音色"相关模块，再定向读源码
2. 用户说"修改用户列表的分页逻辑" → 被动读取 project-summary.md 找到用户模块路径，再定位分页代码
...
**完整触发反例**：
1. 用户只是要求修改一个 Python 文件的函数名 → 不完整触发（局部任务，不需要项目全貌）
2. 用户说"帮我看看这个文件有没有 bug" → 不完整触发（局部任务，不需要项目全貌）

**完全不触发反例**：
1. 用户说"总结一下今天的会话" → 不触发（会话总结不需要项目扫描）
2. 项目无 `.project-context/` 且用户未要求初始化 → 不触发（无索引可读）
...
- 被动读取时，只读取 `project-summary.md` 一个文件，不得触发扫描、初始化或写入操作。
- 被动读取的目的是加速代码定位：从 project-summary.md 提取相关模块路径后，定向读取源码，而非遍历整个项目。
