...
| 查询方式 | 读取 `project-summary.md` | CLI 返回聚焦的 `context_pack` |
...
| DC-4 隐私边界 | v0.6 精打磨 | 索引是否允许保存片段 | 限制摘要和片段大小 |
