...
| 存储方式 | Markdown 摘要为主 | JSON/JSONL 静态索引优先，后续可选 SQLite |
...
```text
在不引入服务器的前提下，增加项目历史和第一版影响分析。
```

交付物：

- `.project-context/index/commits.jsonl`
- `.project-context/index/commit_files.jsonl`
- `forge-memory impact`
- 文件和模块近期变更查询。
- 可解析本地导入关系的反向查询。
- 风险排序信号：
...
| M4 | MCP 包装 | 智能体能调用上下文检索 |
| M5 | 任务驱动框架集成 | 计划展示来自上下文包的文件映射 |
| M6 | Git 和影响分析 | 文件影响包含关系和近期变更 |
...
| DC-2 宿主集成 | v0.2 MCP 细节 | 哪个宿主先消费 MCP | 先接任务驱动框架/cc-switch 工作流 |
| DC-3 语言优先级 | v0.3 关系准确性 | 先强化哪门语言 | TypeScript 优先，再 Dart/Flutter |
| DC-4 存储偏好 | v0.4 | SQLite 是否真的需要 | JSONL 不够用时再加 |
