...
触发条件见 SKILL.md 的"触发纪律"章节，包含自动触发和手动触发两类。

以下情况**不触发**：只是遇到大型项目但不需要上下文、用户要求普通代码修改/文档编写/局部项目分析/会话总结、用户只是说"看一下这个文件/模块""帮我修 bug""解释这段代码"等局部任务。没有命中触发条件时，按常规任务处理。
...
以下异常在步骤 5（读取选中的会话摘要）时检测：

**会话文件损坏**（frontmatter 解析失败或必填字段缺失）：
- 告知用户：`"会话文件 {filename} 格式异常（{具体原因}），无法恢复。"`
- 建议：`"可以：1) 选择其他会话 2) 直接读取 project-summary.md 获取项目全貌"`
- 不自动删除损坏文件，由用户决定

**摘要过时**（`created_at` 距今超过 30 天）：
- 告知用户：`"该会话摘要创建于 {date}，距今已超过 30 天，内容可能与当前项目状态不一致。"`
- 建议：`"建议恢复后结合 recent changes 和源码验证关键事实。"`
- 允许用户继续恢复，但在上下文声明中注明时效性

**index 与文件不一致**（index.md 中记录的会话 ID 在 sessions/ 目录下无对应文件）：
- 告知用户：`"会话索引中有 {n} 条记录找不到对应文件，可能被手动删除。"`
- 自动从 index.md 中移除无效条目，输出清理后的索引
- 继续正常恢复流程
